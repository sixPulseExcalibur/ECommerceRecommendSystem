...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 从预测评分矩阵中提取得到用户推荐列表
    //preRating  數據結構是 （user,product,rating）
    ...
    //從ALS訓練模型model中拿到productFeatures
    //productFeatures的數據結構 ： scala.Tuple2[scala.Int, scala.Array[scala.Double]
    //scala.Int  特徵值
    //scala.Array[scala.Double]  特徵向量  Array要轉換成DoubleMatrix 才能用使用consinSim方法
    ...
    val productRecs = productFeatures.cartesian(productFeatures)
      ...
      //余玄相似度>0.4的保留
      .filter(_._2._2 > 0.4)
      //以productID分組聚合   (用戶id ,((产品1,评分1),(..),(..),(..)))  表现在mongodb里面是一个json数组
      .groupByKey()
      .map{
        case (productId, recs) =>
