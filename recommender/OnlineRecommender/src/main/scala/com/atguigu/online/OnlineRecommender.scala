...
object ConnHelper extends Serializable{
  // 懒变量定义，使用的时候才初始化
  lazy val jedis = new Jedis("hadoop03")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://hadoop03:27017/recommender"))
}
...
object OnlineRecommender {
  ...
  def getTopSimProducts(num: Int,
    ...
    // 从广播变量(隐语义模型中已经计算过的相似度)相似度矩阵中拿到当前商品的相似度列表
    ...
    // 从所有的相似商品中进行过滤  用户ID,产品ID,评分
    // x是相识度列表   productID,score
  ...
  // 计算每个备选商品的推荐得分
  //candidateProducts: Array[Int],   当前商品的备选列表
  //userRecentlyRatings: Array[(Int, Double)],  从redis里获取最近num次评分
  //simProducts: scala.collection.Map[Int, scala.collection.immutable.Map[Int, Double]])  相识度矩阵
  //Array[(Int, Double)]  当前用户的实时推荐列表
  ...
  : Array[(Int, Double)] ={
    ...
    for( candidateProduct <- candidateProducts; userRecentlyRating <- userRecentlyRatings ){
      ...
      if( simScore > 0.4 ){
        // 按照公式进行加权计算，得到基础评分
        //userRecentlyRating._2 => 从redis里获取最近num次评分
    ...
    //_._1 = > (productId, score)
    scores.groupBy(_._1).map{
      case (productId, scoreList) =>
        // scoreList.map(_._2) 所有的第二个参数 即值  sum /scoreList.length  求平均值
        //log(increMap.getOrDefault(productId, 1))  最大值
        //log(decreMap.getOrDefault(productId, 1))  最小值
