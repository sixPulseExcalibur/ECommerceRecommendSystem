...
object ContentRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 2. 定义一个HashingTF工具，计算频次(计算词频)
    //setNumFeatures(800) 使用hash算法映射特征,特征数量. 个数能够涵盖出现的所有词汇就可以了  如果设置的过小  会出现 鼠标 和电子产品对应到一个特征中
    val hashingTF = new HashingTF().setInputCol("words").setOutputCol("rawFeatures").setNumFeatures(800)
    //默认setNumFeatures(212644 2的58次)  由于后面要做笛卡尔积,这里的值最好不要使用默认
   ...
   // featurizedDataDF.show(truncate = false)
    ...
    //稀疏向量 row.getAs[SparseVector]("features")  转成正常数组
    //SparseVector  稀疏向量类的数据格式
    //new DoubleMatrix 稀疏向量转换为矩阵
